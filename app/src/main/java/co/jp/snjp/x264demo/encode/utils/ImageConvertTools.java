...
//        yuv.V = (byte) (0.5 * r - 0.419 * g - 0.081 * b + 128);
     ...
     * 将RGB数组转化为像素数组
     */
    private static int[] convertRgbToColor(byte[] data) {
     ...
     * @param height
     */
    private static void rotationRgbDegree180(byte[] rgbData, int width, int height) {
        for (int h = 0; h < height / 2; h++) {
            for (int w = 0; w < width; w++) {
                int iCoordM = 3 * (h * width + w);
                int iCoordN = 3 * ((height - h - 1) * width + w);
                byte temp = rgbData[iCoordM];
                rgbData[iCoordM] = rgbData[iCoordN];
                rgbData[iCoordN] = temp;
                temp = rgbData[iCoordM + 1];
                rgbData[iCoordM + 1] = rgbData[iCoordN + 1];
                rgbData[iCoordN + 1] = temp;
                temp = rgbData[iCoordM + 2];
                rgbData[iCoordM + 2] = rgbData[iCoordN + 2];
                rgbData[iCoordN + 2] = temp;
            }
        }
    }
    ...
    public static byte[] I420ToJpeg(byte[] src, int width, int height) {
        ...
        rotationRgbDegree180(rgb, width, height);
